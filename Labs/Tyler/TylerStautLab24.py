...
class listThing():
    ...
    def sumList(anyList):       #Def for sum of list
        if sum(anyList) == 0:
            print("0")
        else:
            print("The sum of the list is {0}".format(sum(anyList)))    #Prints sum of list
            
    def avgList(anyList):    #Def for avg of list   
    ...
    def maxList(anyList):   #Def for max of list
        listMax = anyList[0]
    ...
    def minList(anyList):   #Def for min of list
        minimum = anyList[0]
    ...
    def isMember(anyList, anyNum):      #Def for looking for user_input in list
    ...
    def inputs(): #Def for geting the numbers to put into the list
        user_input = int(input("Please enter a number: "))
        myList.append(user_input)
        while user_input != 0:  #Loop to add more numbers
            user_input = int(input("Please enter a number: "))
            if user_input == 0:
                break
            myList.append(user_input)
    ...
    def main():     #Main calls all functions in order
        ...
        inputs()    #Calls program inputs
        print("\n") #Used to seperate parts of program
        printList(myList)   #Prints inputs
        print("\n") #Used to seperate parts of program
        sumList(myList)     #Prints sum of list
        print("\n") #Used to seperate parts of program
        avgList(myList)     #Prints average of list
        print("\n") #Used to seperate parts of program
        maxList(myList)     #Prints the max of the list 
        print("\n") #Used to seperate parts of program   
        minList(myList)     #Prints the min of the list
        print("\n") #Used to seperate parts of program
        
        anyNum = int(input("Enter an Integer to check if it is in the list: "))
        if isMember(myList, anyNum) == True:    #Checks if num in in list
